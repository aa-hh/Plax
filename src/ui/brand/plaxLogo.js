...
// SVG gradient ids must be unique PER RENDERED INSTANCE, not per template:
// several copies of these logos coexist in the DOM (splash + sidebar brand,
// two screens during a route transition), and url(#id) resolves to the FIRST
// match in document order. When that first match sits in a hidden or
// about-to-be-removed subtree, the chevron referencing it silently paints
// nothing — seen on first load as "parts of the logo missing". A counter
// suffix keeps every rendered def self-owned.
var gradSeq = 0;
...
export function plaxWordmarkSvg(opts) {
  ...
  var gid = 'plax-wm-g-' + (++gradSeq);
  return (
    '<svg viewBox="0 0 1015 466" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="plax"' + cls + '>' +
    gradDefs(gid, '888.257', '247', '888.257', '522') +
    '<g transform="translate(-76,-157)">' +
      // gradient chevron behind letters
      '<path d="M887.243 384.838L794 247H889.946L982.514 384.838L889.946 522H794L887.243 384.838Z" fill="url(#' + gid + ')"/>' +
      // p
      ...
      '<path d="M904.335 385L810.87 247H891.5L984.5 385L891.5 522H810.87L904.335 385Z" fill="url(#' + gid + ')"/>' +
    '</g>' +
    '</svg>'
  );
}
...
export function plaxMarkSvg(opts) {
  ...
  return (
    ...
    '<path d="M191.243 192.838L98 55H193.946L286.514 192.838L193.946 330H98L191.243 192.838Z" fill="url(#' + gid + ')"/>' +
    '</svg>'
  );
}
