...
// random per-pixel alpha in [0, 13] (~5% max) over solid white. As the top
// background-image layer it breaks up the smooth radial gradients below just
// enough to avoid visible banding on OLED panels, without ever reading as its
// own visible texture. MUST stay a pure <image> value: `url(...) repeat` is
// background-SHORTHAND grammar — inside `background-image` it invalidates the
// ENTIRE declaration and the whole backdrop silently vanishes. Tiling comes
// from the caller's per-layer `background-repeat` (built via
// cornerWashLayerCount — see detailScreen's detailBgLayerRepeat).
var NOISE_TILE_URL =
