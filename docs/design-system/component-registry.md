...
| nav-scroll glide | 120ms rAF ease-out-cubic (150→120, 2026-07-04 round 2 — user-felt speed) | — | (JS, `focus.js`) |
...
| focus elevation (lift + shadow) | 100ms opacity of pre-rastered `::after` shadow + `translateY(-4px)` in the focus transform | — | ease (`--focus-motion-dur`) |

**Transition choreography (2026-07-04) — the "one thing at a time" rule.** Real-B8 measurement showed the enter fade (declared 200ms) taking 372–1695ms wall-clock, scaling with screen weight — the animations were compositor-fine; the MAIN THREAD was congested by work the screen build kicked off alongside them (poster/hero JPEG decode, JSON prefetch), which is also remote-input dead time. Fix is scheduling, not styling: `src/ui/transitionGate.js` (`beginTransition(ms)` / `endTransition()` / `isTransitioning()` / `onIdle(cb)`) opens a protected window per navigation (router, 400ms, closed early on fade `animationend`) and per home-row cascade (homeScreen, 550ms, closed on last row's `animationend`). Deferred behind it: ALL poster binds (single choke point in `posterImages.js` `bindPosterImage`, placed after the already-bound/already-loading guards so retained posters still reveal synchronously and no concurrency slot can be held by a deferred bind), the detail backdrop colors fetch, and home metadata prefetch. Consumers must queue via `onIdle` — never poll `isTransitioning`. **Round 2 (same day), from the first scoreboard run:** deferral fixed *when*, not *how much* — home still froze ~1.1s building 3 rows synchronously (3 rAF frames in 1.5s) and detail hit a 1950ms worst gap post-gate, the biggest single contributor being the ultrablur backdrop's server-rendered JPEG decode (1212ms, synchronous on Chrome53). So heavy passes are now CHUNKED: home renders ONE row per macrotask (self-schedules the rest through the append path; every pass extends the window, only the final pass hooks the early close), and detail hydrates episode thumbs 6-per-250ms instead of a whole season at once. **Round 3 (2026-07-04, B8-snappy plan Phases 1+2):** the D-pad rail glide (`focus.js` `smoothScrollCarousel`/`smoothScrollVertical`) now also opens the gate — `extendTransition(300)` at each real glide start (never shortens an existing longer window) — so posters gated by `bindPosterImage`'s `isTransitioning()` check don't decode mid-glide; a held key keeps re-extending, so posters resolve once input actually stops (intentional). The idle-queue drain (`flushIdleQueue`) no longer runs the whole queue synchronously — it trickles ONE callback per macrotask (a `setTimeout(0)` chain, guarded by a `draining` flag so the safety timeout and an explicit `endTransition()` can't double-drain), stopping immediately if `beginTransition`/`extendTransition` re-opens the gate mid-drain (remainder stays queued for the next flush). **Round 4 (same day, from the Wave-1 drive session):** the wash fixed the JPEG but detail STILL froze 1.2–1.7s LATE (worstEndedAgoMs:0 — gap still open at window close): the FIFO idle queue was draining the home screen's now-HIDDEN stale poster binds ahead of detail's own images, and the ~4ms setTimeout(0) chain let their decodes batch into one raster frame. Three-part fix: (1) deferred binds re-check at drain time and DROP if the img's host is hidden (`offsetParent === null` — posterImages.js stale-by-hide guard; the owning screen re-primes via the router's retained-re-show `onResume`, which homeScreen now implements); (2) drain ticks are paced `DRAIN_TICK_MS` 48ms (~3 frames) apart so the compositor produces frames between decodes; (3) homeScreen's `onSuspend` now also cancels the pending/in-flight immersive-hero art swap (`ilHeroToken++`) so a 720px hero decode can never land behind a covering screen. **Round 5 (same day):** jank fixed (detail worstMs 1677→327) but CONTENT latency remained — the backdrop swap measured 6s after entry: (1) every DROPPED stale bind still cost a full 48ms drain tick (dozens per scroll burst = seconds of queue delay ahead of the next screen's own deferred work) → drain callbacks may return `false` = "no work done", which chains unpaced; (2) already-queued poster load jobs kept pumping after navigation, burning the 6-per-host connections against the new screen's metadata fetch → `drainPosterLoadQueue` now drops hidden imgs too. New `detail:content-rendered {sinceBuildMs}` breadcrumb separates content latency (network/queue contention) from jank. **Round 6 (same day):** content-rendered measured 431ms ✅ but the wash still landed 2.6s after entry — the chain was gate-wait (~0.8s) + PMS `/services/ultrablur/colors` computing colors on demand (~1.8s). The FETCH now starts immediately on detail entry (network work must OVERLAP the fade, not queue behind it — onIdle-around-the-fetch was a JPEG-era leftover); only the PAINT still respects the gate (queued via `onIdle` if the window is open, else applied directly). Remaining cache-miss floor = the PMS round-trip itself; Phase 3's client-side palette sampling is the planned kill for that server dependency. And the detail backdrop's JPEG-decode leg was removed entirely rather than chunked — `src/ui/colorWash.js` (`buildCornerWashCss`) reproduces the same corner-blended, noise-dithered look natively via CSS `radial-gradient`s + a tiled noise-PNG layer, so the backdrop now paints in ~0ms off the same PMS colors fetch instead of decoding a follow-up image (see the Detail screen entry below). `jank:navigation` gained `worstEndedAgoMs` + `detail:backdrop-swap`/`detail:episodes-appended` breadcrumbs so the worst gap can be placed on the tv.log timeline and attributed; `detail:backdrop-swap` now fires at native-wash-apply time (same breadcrumb name, near-zero-cost instead of anchoring the old JPEG decode). **Round 7 (2026-07-04, B8-snappy plan Phase 3 — layered immersive home hero):** the home hero art is no longer a single crisp top-right box over flat `#131313` — it now drives two more full-screen layers behind the content (ambient corner-wash + soft full-bleed art) off the SAME one-fetch-one-decode bytes, sampled client-side for color (`src/ui/palette.js` blob-XHR → same-origin objectURL → canvas corner-average, dodging the `file://` canvas-taint). Cost per focus-settle is unchanged (still one 720px decode; the objectURL is reused 3× and the palette is a byte-sample, not a second fetch), the crossfades are opacity-only/caps-motion-gated, and `onSuspend`'s existing `ilHeroToken++` already prevents any of the three layers from sampling/crossfading behind a covering screen. New `home:hero-swap {cached}` breadcrumb attributes hero cost (cache-hit color-leads path vs full swap); the soft bleed is `html.caps-motion`-gated + `il--no-bleed` kill-switchable if the B8 GPU-memory watch objects. See the Home Rail entry's "Layered immersive hero" block for the full anatomy. (Phase 3's STRETCH — porting the same client-side palette sampler into detailScreen to kill the ~1.8s PMS `/services/ultrablur/colors` round-trip — was SKIPPED: palette.js's LRU is a module singleton shared with the retained home screen, so a detail-screen `destroy()`/evict could `revokeObjectURL` a URL still referenced by a suspended home layer; the PMS fetch stays the detail backdrop's color source for now.)

**Instrumentation (2026-07-04):** devtools are flaky on webOS 4, so real-device perf visibility runs through the same remote `tvLog` sink used for boot diagnostics (see [[m3-motion-tokens]]). `src/perf/animationTiming.js` (`timeAnimation(el, label, data)`) times the ACTUAL `animationend` wall-clock duration of a CSS animation — not the declared CSS duration — so B8 compositor/decode contention shows up as a longer real number. Wired to: `anim:screen-enter-fade` (router.js, per navigation), `anim:row-stagger-complete` (homeScreen.js, last row's `animationend` = whole cascade done). Payloads split `startDelayMs` (creation→`animationstart`: main-thread congestion / input-dead-time proxy) from `runMs` (`animationstart`→`animationend`: vs the declared duration) — treat `startDelayMs` as the number to budget. `src/perf/frameJank.js` (`sampleFrames`) adds a per-navigation `jank:navigation` scoreboard: rAF gaps over a 1.5s window reported as `{frames, dropped(>33ms), frozen(>100ms), worstMs, busyMs}` — the direct smoothness number; judge every motion change against it instead of feel. The SAME sampler also scores the **sidebar expand/collapse** as `jank:sidebar` (`browsingHubNav.js` `syncExpanded`, `{dir:'expand'|'collapse'}`, 700ms window, fired only on a real expanded-state change). **(2026-07-04 perf pass — the drawer no longer breaks the transform/opacity rule.)** Previously it animated `width` (a LAYOUT property) over 180ms and revealed labels via `display:none→block` (a big reflow) — the measured 323–723ms freeze this sampler exists to prove. Now the **width SNAPS** (one reflow, one frame) and the labels/brand always in the DOM cascade in via compositor `opacity`/`translateX` transitions; the sampler stays exactly where it is to confirm on-device that the snap frame + cascade holds <120ms (Phase 4 target). (Only one sampler runs at a time, so a navigation right after entering the rail supersedes the sidebar window and reports it `superseded:true`.) It ALSO scores the **D-pad rail glide** as `jank:rail-scroll` (`focus.js` `sampleGlide`, `{axis:'horizontal'|'vertical'}`, 500ms window): `smoothScrollCarousel`/`smoothScrollVertical` write `scrollLeft`/`scrollTop` every frame for 150ms (`NAV_SCROLL_MS`) — a per-frame layout — so this captures whether *moving along a rail* holds 60fps, the complement to the pre-existing `input:keydown`/`input:focusCommitted` marks which time only the keydown→focus-commit LATENCY (not glide smoothness) and are `perfMark`-only (need the perf toggle). `sampleGlide` is throttled to one window per burst (a held arrow fires a glide per repeat; new starts within the open window are ignored) so travel reports a single representative number, not dozens of superseded fragments. NOTE: both animation events are main-thread-delivered, so a compositor-driven fade can LOOK smooth while these numbers are big — that still matters (congestion = laggy input), but don't read big totals as "the animation stuttered". `router.js` also adds `screen:contentPainted` — waits for every `<img>` present at screen-build time to finish loading (bounded 4s), reporting elapsed-since-build + images still pending at cutoff, to separate "DOM built" (existing `screen:firstPaint`, one rAF) from "posters actually decoded" (the real B8 jank source). Each channel gates independently: `mark()` (in-memory buffer, `window.__plaxPerf.exportData()`) needs `?perf=1` / `localStorage.plax_perf_enabled` / the Settings "Perf telemetry" toggle; `tvLog()` (remote `tv.log`) needs debug+logSink. `anim:*` and `screen:contentPainted` reach `tv.log` as soon as debug+logSink are on — they do NOT require the perf toggle too. The pre-existing `route:render`/`screen:firstPaint` marks are the exception: still gated behind `isPerfEnabled()` first (unchanged from before this pass), so both flags are needed for those two specifically. When every gate is off, all of this is near-zero-cost (idle listeners, no writes).
...
- **Per-element (as-built vs kit):**
  ...
  - focus: ring via transparent border; under caps-motion (webOS 4+, incl. B8) the wrap scales AND lifts `translateY(-4px)` with an elevation shadow — the shadow is pre-rastered once on an empty `::after` (fixed `0 10px 24px rgba(0,0,0,0.5)`, `border-radius: inherit`, `pointer-events: none`) and focus animates only its OPACITY, never the box-shadow itself (Chrome 53 re-rasters animated blur every frame). Supersedes the former rich-tier-only static lift-shadow (2026-07-04).
...
- **Anatomy (parts → slot), JetStream-mapped:**
  - hero: 2:3 poster (NOT JetStream's full-bleed backdrop — platform 2:3 rule) + info column; backdrop stays as screen bg — **now a native corner-wash** (2026-07-04), not the old ultrablur JPEG: `loadUltraBlurBackdrop` (`src/plex/ultrablur.js`) still fetches the 4 PMS corner colors, but `applyDetailBackground`/`setDetailBackgroundColors` in `detailScreen.js` paint them via `buildCornerWashCss` (`src/ui/colorWash.js`) — 4 corner-anchored `radial-gradient`s + a small tiled noise-PNG layer (`NOISE_TILE_URL`, generated once by `scripts/gen-noise-png.cjs`) for OLED anti-banding, plus `background-color` = the bottomLeft corner so any uncovered center never shows raw surface. The old "upgrade to a server-rendered 1280×720 noise-dithered JPEG" path (`buildUltraBlurImageUrl`/`buildUltraBlurImagePath`/`buildUltraBlurColorGradient`, all removed from `ultrablur.js`) is gone entirely — its synchronous decode measured 1212ms on a real B8 and added nothing the CSS wash doesn't already do near-instantly. `tvLog('perf', 'detail:backdrop-swap')` still fires at wash-apply time (same breadcrumb name, now anchoring a ~0ms paint instead of the JPEG decode).
...
- **Motion (2026-07-04 perf pass — snap + cascade):** the width now **SNAPS on every host** (removed `transition: width` from both the home and library overlay hosts). Animating `width` is a layout property that reflowed every frame for 180ms; on the B8 that measured `jank:sidebar` worstMs 323–723. The designed motion is now the **contents cascading in over the snapped surface** (kit modal-drawer anatomy = surface + scrim appear together, contents choreograph on top): item **labels** and the **brand lockup** are always in the DOM and fade+slide via **compositor-only** `opacity`/`translateX` transitions (`160ms var(--ease-standard-decelerate)`), with a small capped per-item `transition-delay` cascade (0/30/60ms, ≤3 steps, under the 300ms `--dur-medium2` budget). No `display:none↔block` toggle anywhere in the drawer now (that reflow was the freeze). Scrim fade = opacity-only, caps-motion gated (unchanged). Chrome53 rationale: `width`/`display` are the two layout ops that stall the single main thread; every survivor is transform/opacity, gated by `html.caps-motion` (non-caps engines snap, no cascade).
- **Brand lockup invariants (2026-07-04):** the header band is a **fixed** `height: --nav-band-h` (not min-height, no vertical padding) so it can never grow when the lockup swaps; the wordmark svg renders at **68px** so its "x" glyph (275/466 of its viewBox) equals the collapsed mark's **40px** — the x keeps identical optical size AND vertical center across states. **Mark↔wordmark swap is now an opacity CROSS-FADE (perf pass):** both are always in the DOM, `position:absolute` stacked at the same left/vertical origin (mark `left:16px`, wordmark `left:18px` = old +2px margin, both `top:50% translateY(-50%)`), cross-fading `opacity` under `html.caps-motion` — the old `display:none↔block` swap reflowed the header inside the (now-removed) width transition. The host's `overflow:hidden` clips the wider wordmark when collapsed. SVG **gradient ids are per-instance** (`plaxLogo.js` counter): fixed ids duplicated across coexisting copies (splash + rail, two screens mid-transition) made `url(#id)` resolve to a hidden subtree's def and the chevron silently lost its gradient ("parts of the logo missing" on first load).
...
- **Expanded scrim → edge FEATHER (2026-07-04, B8 fix):** the 50vw fixed-gradient scrim layer was only rasterized when its opacity first left 0; on the B8 that first paint queued behind ambient-wash raster work and landed ~3s after the width snap. Replaced by a 140px feather strip hanging off the expanded drawer’s right edge (`::before`, `left: var(--nav-rail-w-expanded)`) — ~8× fewer pixels, rasters within the expand frame. The half-screen dim is gone by design (imperceptible under the opaque drawer surface); rich-tier opacity fade unchanged.
...
- **State model (kit `9:161`, all via existing `:root` tokens):**
  ...
  Icon follows content color via `currentColor` (kit: icon + label share the state color). Collapsed rail: labels `display:none`; the **Selected pill behind the icon** is the current-section cue (`aria-current` + `.active`). **Collapsed pill is a CIRCLE (2026-07-04, corrected same-day):** pass 1 narrowed the HOST's side padding 12→4px in the collapsed state to make room for a 72px circle — this shifted the icon ~8px left on collapse, a visible jump the user flagged ("icon should always stay exactly where it is"). Pass 2: host padding stays the SAME 12px in every state; `--nav-rail-w` was widened 80→96px instead, so the host's content box (96−2×12=72) already equals `--nav-item-h`. The item's pill background covers the full item box, so no item-level padding changes were needed either. **Pass 3 (2026-07-04 perf pass, final):** the item now keeps its **expanded geometry in ALL states** — fixed `width: --nav-rail-inner-expanded` (256px) `flex:0 0 auto`, constant `padding:0 16px`, flex-start — and the collapsed 96px host simply CLIPS it via `overflow:hidden`. This removes the last per-state override (the old collapsed `padding:14px + justify-center`), so the icon x is now **pixel-identical** collapsed vs expanded (28px both, not the ~2px drift of pass 2) AND nothing re-lays-out on the width snap — the whole point of the perf pass. **Pass 4 (same day, merge review — CORRECTS pass 3):** pass 3's 256px-in-all-states item broke the circle: the pill is the ITEM's own background (999px stadium), so clipped at the 96px rail edge it painted as a rounded-left bar with a hard flat cut flush to the rail edge. Final geometry: collapsed items SNAP (no transition — same single reflow frame as the host width snap) to `width: --nav-item-h` (72px = collapsed content box) → the `.active` pill is a true 72×72 circle again. Icon x stays 28px in both states (only the box's right edge moves); the icon sits ~2px left of the circle's optical center (16px pad vs 18px true-center) — imperceptible at 10ft, preferred over a 2px icon jump on expand.
- **Motion:** focus/selected pills are **paint** cues → instant + crisp on every engine. Under `html.caps-motion` the paint swap is eased (`background-color`/`color` `--focus-motion-dur`). **Label reveal (2026-07-04 perf pass):** the label is always in the DOM (no `display:none`) and fades+slides in via a **transition** (not the old `navLabelIn` keyframe) — collapsed = `visibility:hidden; opacity:0; translateX(-8px)`, expanded = visible/`opacity:1`/`translateX(0)`, `opacity+transform 160ms var(--ease-standard-decelerate)` with a 0/30/60ms per-item cascade (≤3 steps) — this cascade IS the drawer's felt motion now that width snaps. **Fixed-width clip (perf pass, as corrected by Pass 4):** `.browsing-hub-item` is `width: --nav-rail-inner-expanded` (256px = expanded inner box) `flex:0 0 auto` in peek/expanded, SNAPPING to `--nav-item-h` (72px) when collapsed (both snaps land in the host's single reflow frame — no width transition anywhere); padding/justify never change per state, so the icon x-position is **pixel-identical** collapsed vs expanded (host pad 12 + item pad 16 = 28px both) and the collapsed `.active` pill is a true 72×72 circle (see the circle note above). **No transform-scale on the item** — the kit's `focusedScale 1.05` is omitted because Chromium 53 blurs GPU-scaled text (same ratified rule as text Buttons); the pill is the cue and capable HW simply runs the eased paint/label motion at higher FPS.
...
- **Layered immersive hero (2026-07-04, Phase 3 — `feat(home): layered immersive hero`):** the flat `#131313` behind the home content is replaced by THREE full-screen layers behind everything (bottom→top: **ambient wash < soft bleed < sidebar/feed/hero content**), all inside `.home-layout` (position:relative on `.screen-home`). Anatomy (`src/ui/screens/homeScreen.js` `ilUpdateHero`/`ilApplyAmbient`/`ilShowHero`; `.il-ambient`/`.il-hero__bleed`/`.il-hero__bleed-scrim` in app.css):
  ```
  .home-layout (position:relative; z-index:0 — an explicit STACKING CONTEXT: merge review corrected the layers from positive to NEGATIVE z, because a z:1 layer paints ABOVE non-positioned content text (CSS2.1 App. E) and veiled the whole feed)
    .il-ambient--a / --b       ← z:-3 two crossfading corner-wash layers (buildCornerWashCss(palette)), max opacity 0.55, subtle TINT not wallpaper
    .il-hero__bleed--a / --b   ← z:-2 two crossfading full-bleed `cover` art layers (SAME objectURL as the crisp box), max opacity 0.45 — 720→1920 upscale = intended softness
    .il-hero__bleed-scrim      ← z:-1 strong flat scrim (0.55 base, →0.85 bottom + left text column) keeping rail text/cards legible
    .browsing-hub-nav-host     ← z:5  (unchanged)
    .home-main > .il-hero      ← the EXISTING crisp top-right 16:9 box + scrim + content column. Its own `background` was made TRANSPARENT (was opaque surface-dim) — an opaque content-wrapper background occludes the negative-z layers across the whole hero band (the "focus does nothing" bug); the ambient wash now IS this band's neutral base, and body surface-dim shows through before first focus. Any future opaque bg on a home content wrapper (`.home-main`/`.home-feed-host`/`.il-hero`) will re-break the immersive layers.
  ```
  **Color source (revised — the shipped path):** the ambient wash colors come from PMS `/services/ultrablur/colors` via `loadUltraBlurBackdrop` (the SAME source the detail backdrop uses, proven on-device; home prefetches it on focus at 250ms so the 500ms settle is usually cache-warm), and the crisp box + bleed use the plain art URL as a CSS background via an `<img>` preload (display only, no CORS/canvas — how the box worked before Phase 3). The originally-shipped client-side sampler (`src/ui/palette.js` `getPalette(url)` blob-XHR (dodging the `file://` canvas-taint: cross-origin `<img>`→canvas `getImageData` throws SecurityError; a `blob:` objectURL is same-origin so the read is allowed), samples the 4 corner colors off an 8×8 downscale (2×2 corner blocks), and returns `{colors, objectUrl}`; the objectURL was to feed the crisp box + bleed + palette) is PARKED unused: on the B8 its cross-origin blob-XHR/canvas path produced null and took the ENTIRE hero (incl. the previously-working crisp box, which Phase 3 had rewired onto the objectURL) blank — "focus does nothing". palette.js + its tests remain for a future CORS-validated revival, but nothing visible depends on it. `home:hero-swap {colorsFrom:'pms'|'none'}` breadcrumb reports whether the wash colors resolved. LRU cache (max 12, sized > the 6-entry `ilCacheKeys` so an on-screen URL can't be evicted while current) `revokeObjectURL`s only on evict + on screen `destroy()` (`clearPaletteCache`). **Choreography = "color leads, image follows":** on settle `getPalette` resolves ambient first (near-free on cache-hit), then the crisp box + bleed crossfade together off the objectURL — all guarded by `ilHeroToken` + `destroyed` (the same supersede pattern the old single box used; `onSuspend` bumps the token so nothing samples/crossfades behind a covering screen, `onResume` restores the current ambient side). **Kill-switch + gate:** the bleed layers are gated behind `html.caps-motion` AND suppressed by an `il--no-bleed` class on the screen root — ambient wash stays. **`il--no-bleed` is ACTIVE (2026-07-04, DESIGN verdict from on-device review, not perf):** the full-bleed layer read as a duplicated poster echo behind the crisp box — a 720→1920 upscale is not soft enough to register as ambient glow. Shipped anatomy = ambient wash + crisp subject box. Re-enable only with a REAL blur source (e.g. server-blurred art), never the raw upscale. Both suppress rules carry `!important` (merge review): the swap writes opacity/background-image INLINE, and inline style beats plain class rules — without `!important` the gate and kill-switch silently lose. **Motion:** opacity-only crossfades (`var(--dur-medium1) var(--ease-standard)`), caps-motion gated; longhand offsets, no layout/paint animation (Chrome53-safe). **Breadcrumb:** `home:hero-swap {cached}` fires at swap-commit time (`cached` = palette LRU hit) so hero cost is attributable on the tv.log timeline. Backend-agnostic (bytes→canvas; no server color API), so it dresses sparse Jellyfin/new-user homes too. **Edge-melt fix (same day, on-device screenshot review):** the crisp box's left/bottom edges used to fade via a hardcoded PAINTED `var(--surface-dim)` overlay, assuming `.il-hero`'s surroundings were always that same flat color — true before this phase, false now that `.il-hero` is transparent so the ambient wash shows through. The mismatch (painted flat color vs. the true ambient-tinted background) produced a visible hard seam at the box boundary. Fixed by fading the IMAGE ITSELF to transparent via `-webkit-mask-image` on `.il-hero__backdrop` (two intersecting directional gradients, `destination-in` composite — old WebKit, Chrome53-safe); `.il-hero__scrim` now only adds light text-legibility darkening and fades to TRUE transparency, never a claimed color. Correct regardless of active theme or ambient tint since nothing is hardcoded to match. **Seam fix round 2 (same day, on-device):** transparency-only melts still left a color-temperature boundary (dark photo pixels handing off to an averaged wash). `ilTintHeroScrim` (homeScreen.js) now rebuilds `.il-hero__scrim`’s gradient INLINE per swap from the item’s own `topRight` palette color (clear 28% → 0.45 mid → 0.35 at the box edges, both axes): the tint rises over the same band the mask fades the photo, hiding mid-band ghosting and keeping hue continuous into the wash’s top-right radial. Static CSS scrim = pre-colors default only. (NOTE: the `{cached}` breadcrumb field name above is stale from the palette-sampler era — the shipped code emits `{colorsFrom:'pms'|'none'}`, see the Color-source note earlier in this entry.)
