...
- **Status:** ✅ to-spec (with ratified 10-ft deviations) · 2026-06-22 — the one overlay primitive is now in place. `openSidePanel` (single + multi-select) + `openActionDialog` cover every transient option/confirm overlay; the bespoke `openModal` + `.detail-modal-option/-list/-title/-empty` + `.gt-modal-option*` are deleted. Remaining `.detail-modal*` rules (`.detail-modal`, `[hidden]`, `-sheet`, `-footer`, `-cancel`) are reused **only** by the ratified `openTextInputModal` Text-field surface (left intact). Player track-selector / media-info drawers + autoplay toast = already kit-correct / ratified non-modal (left).
- **Code (as-built):**
  - **Side panel (option/selection list)** — `openSidePanel()` in `src/ui/components/controls.js` (kit Direction=Right). Builds the floating `.player-track-modal*` surface (radius 16 `--bg-surface` `--space-5` p, kit dark/3 shadow, right-edge `--drawer-edge-gap`, bottom-anchored, `--drawer-side-min-h` floor) + reuses the kit-correct `.player-menu-option` rows (List Item `561:3969`: height 64 / radius 8 / 12-16 / 24px `control`). Body-level overlay with self-contained UP/DOWN(+footer)/Back/LEFT-RIGHT-trap D-pad (same trap as the player sheet). Aliased `.gt-side-panel*` classes carry the multi-button footer layout.
  - **Action dialog (confirm/prompt ≤2 actions)** — `openActionDialog()` (kit Direction=Bottom); `.gt-dialog*` in `src/styles/app.css` (tokens `--drawer-dialog-h 200` / `--drawer-actions-w 268` / `--drawer-side-w 280`). Heading `--font-row-label`, desc `--font-body`, Primary=`.btn-primary` / Secondary=`.btn`, self-contained UP/DOWN + Back D-pad, body-level overlay.
- **Side-panel single vs multi-select:**
  - **single-select (default):** rows `role=radio` in a `role=radiogroup`; picking fires `onPick(id)` and closes; selected = soft `--active` fill + filled accent 24px radio control (`.player-menu-option-check`). Footer = one `.btn-outline.btn--sm` Cancel. Used by detail Subtitles/Quality/Episode pickers (`detailScreen.js`).
  - **multi-select (`multiSelect:true`):** rows `role=checkbox` in a `role=group`, `aria-checked` toggled in place; picking fires `onToggle(id,opt,nowChecked)` and **stays open**; checked = same `--active` fill + a **24px checkbox control** (`.player-menu-option-check--checkbox` — square radius-6, accent-filled box + tick drawn from `::after` via `rotate(45deg) scale()`, transform/opacity-only = Chrome53-safe; focused+checked inverts to dark box + light tick to read on the light-fill focus row). Footer via `footerActions:[{label,className,onSelect,keepOpen}]` — supports a `keepOpen` action (e.g. "New list", `.btn`) + a closing "Done" (`.btn-outline.btn--sm`). **Consumer:** `watchlistBookmark.js` `openWatchlistPicker` — each watchlist row toggles add/remove the title; "New list" creates+adds then closes+reopens to rebuild the row set (now checked); "Done"/Back closes.
- **Migrated:**
  - ✅ **resume-choice** → `openActionDialog` (kit Bottom dialog). Bespoke `.detail-modal` markup deleted.
  - ✅ **detail Subtitles / Quality / Episode pickers** → `openSidePanel` (single-select). `openModal` retired.
  - ✅ **watchlist "Delete list?" confirm** (`watchlistScreen.js`) → `openActionDialog` (was an `openModal` 2-option list; a ≤2-action confirm belongs on the action dialog).
  - ✅ **watchlist bookmark picker** (`watchlistBookmark.js`, fix-list item 4) → `openSidePanel({multiSelect:true})`. Last bespoke `.detail-modal` picker; its markup + the dead `.detail-modal-option/-list/-title/-empty/-list--episodes/-list--info` + `.detail-network-info-line` + `.gt-modal-option*` CSS and `.detail-modal-option` focus-selector entry all deleted.
...
- ✅ ~~**Modal drawer** (bespoke per-screen `.detail-modal` / `openModal` pickers vs the one kit drawer primitive)~~ — resolved 2026-06-22: `openSidePanel` (kit Direction=Right side panel, single **+ multi-select checkbox** variants reusing the kit `.player-menu-option` List-Item rows) + `openActionDialog` (kit Direction=Bottom) now cover every option/confirm overlay. Detail Subtitles/Quality/Episode + watchlist-bookmark multi-select migrated to side panel; watchlist delete-confirm → action dialog; `openModal` + all dead `.detail-modal-*`/`.gt-modal-option*` CSS deleted (only `openTextInputModal`'s Text-field surface keeps `.detail-modal`/`-sheet`/`-footer`/`-cancel`).
- Standing 🚧 fix-lists: none open. (Search input, Tabs, Nav item, Player track-selector sheet, Progress bar ✅ reconciled 2026-06-20; Modal drawer ✅ reconciled 2026-06-22.)
