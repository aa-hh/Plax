...
- **Status:** 🚧 in progress · 2026-06-22 — **one overlay primitive achieved for option lists**: `openSidePanel()` factory built + all four bespoke `openModal()` callers migrated; dead off-kit picker CSS/JS deleted. Player sheets kept as-is (well-tested focus JS, low-risk path). Remaining 🚧: `watchlistBookmark` multi-select picker still hand-rolls `.detail-modal` (multi-select toggle, out of the single-select side-panel form) — see fix-list.
- **Code (as-built):**
  - `openSidePanel()` (kit **side panel**, Direction=Right) in `src/ui/components/controls.js` — the shared option-list primitive. Renders `.gt-side-panel` (scrim @60%, explicit edges) → `.gt-side-panel-sheet` (surface-container `--bg-surface`, radius 16 `--radius-xl`, `--space-5` pad, kit dark/3 shadow, content-sized `min-height --drawer-side-min-h`/`max-height 100%`, width `--modal-sheet-max-w`) → borderless `.gt-side-panel-header`/`-title` (kit List/Header pt-8 pb-16 px-16) → `.gt-side-panel-list` (role=radiogroup, flex scroll region) of **`.player-menu-option` rows** (reuses the ✅ Player-track-selector kit List-Item anatomy: min-height 64, radius 8, padding 12/16, 24px radio `control`, single-line label, light-fill `:focus` inversion, soft `--active`) → optional `.gt-side-panel-footer` with a `.btn .btn-outline .btn--sm` Cancel. Self-contained UP/DOWN + Back D-pad, LEFT/RIGHT trapped, body-level overlay. opts accept `{header|title, rows|options, onSelect|onPick, selectedId, footer, cancelLabel, onCancel}`. Slides in from the right under `html.caps-motion` (`gt-drawer-right-in`), instant otherwise.
  - `openActionDialog()` (kit Bottom dialog) in `controls.js`; `.gt-dialog*` in `src/styles/app.css` (tokens `--drawer-dialog-h 200` / `--drawer-actions-w 268` / `--drawer-side-w 280`). Heading `--font-row-label`, desc `--font-body`, Primary=`.btn-primary` / Secondary=`.btn`, self-contained UP/DOWN + Back D-pad, body-level overlay.
- **Migrated:**
  - ✅ **detail Subtitles / Quality / Episode pickers** + **watchlist delete** → `openSidePanel` (kit side panel). The bespoke off-kit `openModal()` (`.detail-modal`/`.gt-modal` picker — row min-height 50, radius 0 + 3px accent bar, padding 21/24, blue-soft Selected + bare `✓`, `.detail-modal-cancel`) and its CSS (`.gt-modal-option*`, `.detail-modal-cancel`) were **deleted**. `openModal` is gone from the codebase.
...
- **Platform reconciliation (applied):** Material 3 **blue** active/selected accents (not literal kit); type up-scaled to the app's 10-ft sizes (title `--font-body` 24); **no `:focus-within`** — selected/active via JS classes; slide-in via transform-only (`html.caps-motion .gt-side-panel-sheet → gt-drawer-right-in`) else instant; rows reuse the ✅ Player-track-selector `.player-menu-option` (already kit-List-Item-correct); actions/cancel reuse `.btn`/`.btn-primary`/`.btn-outline`/`.btn--sm`; `--z-player-overlay` + 2; width via `--modal-sheet-max-w`; min-height `--drawer-side-min-h`; edge inset `--drawer-edge-gap`.
- **Deviations from kit — remaining fix-list (🚧):**
  1. ✅ ~~Extract a shared `openSidePanel()` factory~~ — done 2026-06-22 (`controls.js`); `.gt-side-panel*` chrome + reused `.player-menu-option` rows.
  2. ✅ ~~Migrate the `openModal()` callers (detail Subtitles/Quality/Episode, watchlist delete) to the side panel~~ — done 2026-06-22; `openModal` deleted.
  3. ✅ ~~Delete dead bespoke `.gt-modal-option*` / `.detail-modal-cancel` CSS + `openModal` JS~~ — done 2026-06-22. **Partial:** `.detail-modal` / `.detail-modal-sheet` / `.detail-modal-title` / `.detail-modal-list` / `.detail-modal-option*` / `.detail-modal-footer` / `.gt-modal`/`.gt-modal-sheet` are **retained** — still used by the two remaining `.detail-modal`-based surfaces (the `openTextInputModal` text fields in `controls.js` + `settingsScreen.js`, and `watchlistBookmark.js`).
  4. ✅ ~~`.player-menu-option` `gap` Chrome53 nit~~ — removed 2026-06-22 (space-between handles spacing).
  - **Still open (🚧):** **`watchlistBookmark.js` multi-select picker** still hand-rolls `.detail-modal` (toggle add/remove across N lists + "New list" + "Done" — a multi-select, not the single-select radio form `openSidePanel` provides). Migrating it needs a side-panel multi-select (checkbox `control`) variant; deferred. Until then the `.detail-modal*` picker CSS stays. (Entry stays 🚧 solely for this.)
