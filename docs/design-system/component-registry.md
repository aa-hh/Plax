...
| nav-scroll glide | 150ms rAF ease-out-cubic (0ms under reduced motion) | — | (JS, `focus.js`) |
...
| cross-screen fade-through | hard cut out → 200ms in (`--dur-short4`); retained Back re-shows snap (router strips `screen-host--enter`) | — | standard-decelerate |
| nav drawer expand | width 200ms (`--dur-short4`, layout by ratified exception, overlay screens only); labels + wordmark 150ms (`--dur-short3`) with 25ms sibling stagger capped at 75ms | snap | standard / standard-decelerate |
| nav drawer scrim | 150ms (`--dur-short3`) | 150ms | standard |
| loading overlay (playback / buffering) | 150ms delay then 200ms in (`--dur-short3` + `--dur-short4`); `loadingOverlay.js` holds a painted overlay ≥400ms | 150ms (`--dur-short3`) | standard-decelerate / standard-accelerate |
| splash → first screen | — | 300ms (`--dur-medium2`), DOM removal at 350ms | standard-accelerate |
| detail `•••` spring menu | 200–250ms + stagger | short | spring (overshoot) |

### D-pad navigation engine (foundations)

- **Status:** ✅ · 2026-07-04 (glide: reduced-motion path 2026-09-20)
- **Scroll glide:** `NAV_SCROLL_MS` 150ms rAF ease-out-cubic per rail/feed; a new request on the same scroller cancels the in-flight glide (held D-pad chases focus); Magic Remote clicks skip the snap (300ms recent-pointer guard); `prefers-reduced-motion: reduce` makes it an instant jump (`glideMs()`).
...
- **States (hardened 2026-09-20):**
  - **poster loading:** `<img>` at opacity 0 until decoded, then `.poster--loaded` fades it in — `--dur-short4` / `--ease-standard-decelerate`, gated on `html.caps-motion` (hard cut without it; `prefers-reduced-motion` drops the tween).
  - **no art / load failed:** the wrap gets `.card-poster-wrap--no-art` (surface fill + 1px inset hairline, `background-clip: padding-box` so it stays inside the ring border). Set at create time when the item has no thumb (`mediaCard.js`) and from the shared load path on `onerror` (`posterImages.js` `setPosterWrapNoArt`), so deferred/viewport hydration gets the same fallback; a later successful load clears it. The `<img>` never shows the browser's broken-image glyph. (The old fallback hid the `<img>` only on the initial bind, leaving deferred failures as see-through holes.)
  - **focus + reduced motion:** ring/border snaps on; only the scale transition is removed.
  - **text:** title/subtitle/meta are single-line ellipsis; 120-char and CJK strings truncate, never wrap. Missing year/rating/duration simply omit that line (`formatCardLines`).
...
- **States + content limits (hardened 2026-09-20):** titles clamp to 2 lines with `word-break: break-word` (movie/show/episode/season — 120-char and CJK titles never push Play below the fold); summaries clamp to 4 lines (movie/show), 5 (season), 3 (episode); credit values, breadcrumb labels (`.detail-breadcrumb-trail__label`, 520px) and the Up Next title (`.detail-up-next-btn__title`, 720px) ellipsise on one line. Missing art → `.detail-poster.poster--missing` shows the surface placeholder (the shared poster fade otherwise leaves an unbound `<img>` invisible); a cast photo that fails to load swaps to the initials disc (`bindPosterImage` `onError`). Empty related rails collapse (`.detail-rails--movie:empty`). **Load error** → `.detail-load-error`: plain-voice message from `friendlyLoadError(err, action)` ("Could not load this title. Check the server connection and try again." / 401 / 403 / 404 / 5xx / timeout variants) + a focused **Try again** button (`#btn-detail-retry`). Seasons/episodes/rails failures render the same message + retry inside their host (`renderHostStatus`, `.detail-episode-grid-status` / `.detail-episode-grid-retry`); pressing retry parks focus on the active tab or Play before the host re-renders. Empty copy: "No seasons on the server for this show yet." / "No episodes in this season yet."; `showMetaLine` pluralises ("1 episode"). **Concurrency:** one `renderGen` guards every path that ends in `renderDetail` (initial load, retry, refresh, watch toggle) so a slow older response can't overwrite a newer render; the watch toggle takes a thunk and drops presses while one is in flight (`watchActionPending`) — nothing is changed optimistically, so a failure leaves the screen as-is and names the action ("Could not mark as watched…"); `ensureSeasonEpisodesLoaded` shares one in-flight promise (Play-on-season during the Up Next lookup no longer gets an empty list). Every dynamic re-render calls `invalidateFocusableCache()`.
...
- **Focus (D-pad):** items ship `hidden` (non-focusable per `isNavFocusable`) until opened. Open → un-hide + `is-open`, focus the nearest item, set trigger `data-nav-up`→nearest. Up/Down walk the stack via per-item `data-nav-up`/`-down` (resolved before geometry); top item self-traps Up, bottom item Down→trigger; Left/Right trapped to self. Back (461/27/8/Backspace/GoBack) intercepted capture-phase → close + return focus to trigger; also closes on `focusout` of the container. No `:focus-within`. Uses `data-nav-*` attributes (die with the DOM) rather than `addNavOverride` (which `clearNavOverrides` would wipe globally). **Hardened 2026-09-20:** selecting an item returns focus to the trigger *before* the stack hides (focus no longer collapses to `<body>` for a beat); the capture-phase Back listener is registered in the screen's `cleanups` list and removed on `destroy()` (it used to outlive a screen left with the menu open).
...
- **Disabled-while-loading (added 2026-09-20):** `.library-filter-chip[aria-disabled="true"]` (and `.library-scan-btn[aria-disabled="true"]`) = 45% opacity, still focusable, focus inversion shows through; the screen's click handlers ignore the press. Never the native `disabled` attribute on a 10-foot control — it makes the element unfocusable and drops D-pad focus to `<body>` mid-load. Selected+focused+disabled composes: blue fill → light inversion → dimmed.
...
- **Motion (2026-09-20 tokens):** width-expand eased (`width var(--dur-short4) var(--ease-standard)`, 200ms) **only on the overlay screens** (reflows just the rail's own items); in-flow screens snap (animating width there reflowed the heavy main content — the "laggy entry" stutter). Labels AND the brand wordmark slide in via `@keyframes navLabelIn` under `html.caps-motion` (`--dur-short3` 150ms, `--ease-standard-decelerate`, `backwards` fill) with a sibling stagger of 25ms per item capped at 75ms from the 4th item (worst case 225ms, under the 300ms cap). Scrim fade = opacity-only, `--dur-short3` `--ease-standard`, caps-motion gated. Reduced motion: width snaps, label/wordmark reveal becomes opacity-only.
...
- **Motion:** focus/selected pills are **paint** cues → instant + crisp on every engine. Under `html.caps-motion` the paint swap is eased (`background-color`/`color` `--focus-motion-dur` `--ease-standard`) and the label slides in (`navLabelIn`, `--dur-short3`, staggered — see the drawer entry). **No transform-scale on the item** — the kit's `focusedScale 1.05` is omitted because Chromium 53 blurs GPU-scaled text (same ratified rule as text Buttons); the pill is the cue and capable HW simply runs the eased paint/label motion at higher FPS.
...
- **Feed states (hardened 2026-09-20):**
  - **error → retry:** every hub loader (`loadHomeHub`, `loadWatchlistHub`, `loadLeavingSoonHub`) renders `renderFeedError`: a `.status-msg` ("Could not load Home. " + `homeFeedErrorMessage(err)` — 401 / 403 / timeout / 5xx / network each get plain-voice copy; raw `err.body` never reaches the DOM) and a `.btn.home-feed-retry` "Try again" that re-runs the loader. Focus lands on the button unless the user is deliberately in the sidebar (`focusFirstFeedCardIfNeeded`); the hero is hidden so no stale backdrop sits above the message. Leaving Soon used to show its *empty* copy on error and Watchlist had no catch at all (stuck on "Loading…").
  - **empty:** `renderFeedEmpty` — message only; if the re-render dropped focus, it lands on the sidebar (never `<body>`).
  - **entrance:** the status message + retry fade in with `screen-enter-fade` (`--dur-short4` / `--ease-standard-decelerate`) under caps-motion; hard cut without.
  - **row reveal + reduced motion:** `.row-section--enter` keeps the opacity fade but drops the 16px rise under `prefers-reduced-motion: reduce`.
  - **hero missing art:** a focused card with no `art`, or whose art fails to fetch, fades the previous card's backdrop out (both layers at 0 → surface-dim) instead of wearing the old art under the new title; token-guarded so a slow failure can't clobber a newer card. Crossfade is `--dur-medium2` / `--ease-standard`, gated on caps-motion (was an ungated 350ms bare `ease`).
  - **hero text:** title clamps to 2 lines and overview to 3 via `-webkit-line-clamp` (the old `max-height` caps let a 7px / 2px sliver of the next line show); `overflow-wrap: break-word` so unspaced CJK / 120-char titles wrap. Empty `.il-hero__label` / `.il-hero__meta` collapse (`:empty { display:none }`) so a missing type/year leaves no phantom margin.
  - **long rails (1000+ items):** `virtualRow.render()` calls `invalidateFocusableCache()` after rebuilding its DOM slice — without it the D-pad only knew the detached cards and dead-ended once a rail longer than `visibleCount` (20) shifted its window.
...
- **States (hardened 2026-09-20; shared renderers in `src/ui/screens/screenStates.js`, tested in `test/screen-states.test.js`):**
  - **Loading:** 12 `.row-skeleton-card.media-grid-skeleton-card` placeholders on the grid's own 14px gutter (same footprint as cards → no jump on load). Chips + Scan get `aria-disabled="true"` (dimmed to 45%, still focusable; clicks ignored) — never `disabled`, which would drop D-pad focus to `<body>`.
  - **Loading the rest (progressive, 1000+ items):** `.library-status` line reads "Showing the first N titles while the rest load…", then the final count. If the rest fails: "Only the first N titles loaded. <problem + recovery>" + inline `Load the rest` button (`.library-status__retry`) that refetches only the remainder.
  - **Empty:** in-grid `.status-msg` — "Nothing in <lib> yet. Add media on the server, then Scan library." / with the Unwatched filter: "Everything in <lib> is watched. Choose All to see the full list." Focus lands on the `All` chip (never the overlay rail).
  - **Error:** in-grid `.status-msg--error` from `describeLoadError` (401/403/404/5xx/timeout/network each name the problem and the recovery) + `.status-retry` "Try again" button; focus lands on it.
  - **Scan:** in-flight = label "Scanning…" + `aria-disabled`; done = "<lib> refreshed: N titles."; error = `friendlyScanError` copy + "Try again" in the status line. Replacing the status line while its retry button is focused parks focus on the Scan button first.
  - **Title:** `.library-title` is one line with ellipsis (server-authored names up to 120 chars / CJK).
- **Transitions:** dataset change (load / filter / sort / empty) replays `.media-grid--enter` — opacity-only fade, `--dur-short4` (200ms) `--ease-standard-decelerate`, gated on `html.caps-motion`; snaps without it; nothing to reduce under `prefers-reduced-motion`. Loading skeleton appears instantly (no exit animation on the outgoing content).
...
- **Per-element spec** — kit value · as-built (`src/styles/app.css`) · resolution:
  ...
  | **show/hide** | JetStream slide-in/fade-in, focus requested on show | **M3 asymmetric Fade** (2026-06-30): enter `opacity var(--dur-short4) var(--ease-emphasized-decelerate)` (200ms, emphasized-decelerate); exit `opacity var(--dur-short3) var(--ease-standard-accelerate)` (150ms, standard-accelerate — aligned 2026-09-20 to the Motion pattern map's overlay/transport row; was emphasized-accelerate) + delayed `visibility 0s linear var(--dur-short3)`. Transport/meta toggled by class. **Skip-prompt-active hide (2026-09-20):** `.player-overlay--hidden.player-overlay--skip-intro-active` keeps the overlay at `opacity:1` and fades `.player-bottom` instead (same 200/150ms tokens) — the prompt is a child of the overlay, so the overlay-level `opacity:0` had been fading the prompt out with the transport (regression from the 2026-06-30 retime; `test/player-overlay-css.test.js` guards it). | ✅ **to-spec for platform** — opacity-only transition is Chrome53-safe; the slide is intentionally dropped (ratified — class toggle, transform/opacity only, no layout anim). Retimed from `0.22s ease` symmetric to M3 asymmetric (exit shorter than enter — M3 rule); ungated (runs on all engines, compositor-cheap). Focus defaults to play/pause on open (`focusOverlayDefault`). **Wake on D-pad (2026-09-20):** any arrow press while hidden re-shows the overlay and is consumed (`handlePlayerArrowWake`), so the first press never also moves focus among invisible controls. |
  ...
  | **skip intro/credits prompt** | composes kit Button Filled `169:1649` | `.player-skip-intro-prompt` bottom-center pill, `--button-container` fill, `--gt-radius-button`, label `--font-meta`/`--gt-weight-label`; selected(auto-focus)=light-pill inversion via `.btn:focus`; credits countdown = `transform:scaleX` fill overlay; **enter** `player-skip-prompt-in` (opacity + 12px rise, `--dur-short4` emphasized-decelerate) under `html.caps-motion`, snap otherwise; exit = instant `[hidden]` (entrance-only, like the drawers); `prefers-reduced-motion` keeps the fade, drops the rise | ✅ **to-spec** (reconciled 2026-06-20; enter motion added 2026-09-20). Auto-focused when shown → renders inverted ("selected"); OK-hint flips dark-on-light. Countdown fill is transform-only and deliberately **ungated** — it is the time-remaining indicator, not decoration. **Data dep:** markers require `getMetadata` `includeMarkers=1`/`includeChapters=1` AND server-generated markers. |

- **Auto-hide / Back behavior (resolved):**
  - Idle auto-hide **3000ms** (`OVERLAY_HIDE_MS`); suppressed while a menu/info/media-info/autoplay panel is open, the motion cursor is visible, **or a playback failure is showing its Retry** (`scheduleOverlayHide` guards — the message + Retry live in the overlay, so the recovery never auto-hides). On hide, if a skip prompt is active it takes focus.
  - **Hardening (2026-09-20):** buffering scrim is debounced (`BUFFERING_SHOW_DELAY_MS` 250ms — sub-250ms stalls never flash the full-screen dim; hide is immediate; `html.caps-motion .loading-overlay` fades in 150ms standard-decelerate). Rebuffer-timeout messages with no fallback re-show the overlay so the status line is seen. Up-next toast (`.player-autoplay-panel`) focuses **Play now** on open, re-homes focus to play/pause on cancel/complete, and enters via `player-autoplay-in` (opacity + 8px rise, short4 emphasized-decelerate, gated, reduced-motion → fade only). prev/next pills at queue edges render `:disabled` at 38% opacity. Unknown duration renders `--:--` and a 0% bar (never a full bar). Track-menu rebuilds call `invalidateFocusableCache()`. Scrub preview hides the thumb on image error (time-only). Input listeners (`keydown`/cursor/pointer) detach synchronously in `destroy()` before the async progress flush.
...
- **Search screen states (hardened 2026-09-20, `src/ui/screens/searchScreen.js`):** idle = "Type to search your libraries."; searching = 3 `.row-skeleton` rails + `aria-busy="true"` on the input; 0 results = "No results for “<query, cut at 60 chars, emoji-safe>”. Try a shorter title or a different spelling."; error = `.status-msg--error` from `describeLoadError` + `.status-retry` "Try again" (re-runs the last query; DOWN from the input reaches it); no server = "No server connected. Sign in from Settings, then search again." Every results re-render first moves focus back to the input if it was inside the results pane, so a wiped card/retry button never leaves focus on `<body>`. Concurrency: 350ms debounce + a request token; any response (streamed row, final rows, or error) whose token is stale is dropped. Rows that arrive in one batch reveal with the Home `.row-section--enter` stagger (40ms step, capped at 6); streamed rows reveal individually with no delay.
...
- **Error state (added 2026-09-20):** `.login-field--error` on the field wrapper + `span.login-field__error` (22px `--color-error-light`, `[hidden]` when clear) directly under the field. Rest outline → `--color-error-light`; the `:focus` rule outranks it so the focused field stays blue. The typed value is preserved. Set/cleared by `setFieldError()` in `jellyfinLoginScreen.js`; cleared on every edit of that field.
...
- **Anatomy:**
  ...
  ```
- **State machine:** Read = read row visible, editor `[hidden]`, only the read row is a focus stop. Editing = editor visible, read row hidden, focus contained input↔Save/Cancel/Test. Transitions: select row → reveal editor + focus/select-all input (raise keyboard) · Enter/Up/Down on keyboard → focus Save · Save → validate, then commit + back to read · Cancel/Back/Esc → revert + back to read.
- **Validation + inline feedback (hardened 2026-09-20):** `validateLogSinkUrl(value)` (exported from `settingsScreen.js`, unit-tested in `test/settings-hardening.test.js`) gates Save and Test: empty clears the setting; otherwise must start `http://`/`https://`, have a host, no whitespace, ≤ 2048 chars (`maxlength` on the input too). Errors render in `p.gt-settings-editor__msg--error#log-sink-msg` directly under the field (`aria-describedby`), the input gets `aria-invalid="true"` (border → `--color-error-light`), the typed text is preserved and the cursor returns to the input. The top-of-page `.settings-status` line is NOT used while editing — the field sits at the bottom of a long scroll, so that line is off-screen.
- **Test states:** idle → pending (`Test` button `aria-busy="true"` + label "Testing…", message `--pending` "Sending a test line to <url>…") → ok (`--ok`, "Receiver replied OK…") | error (`--error`; HTTP status / unreachable / 5 s timeout, each naming the fix and ending "Then press Test again"). One ping at a time (second press ignored while busy); Cancel/leave aborts the XHR and a request counter drops stale replies. Focus stays on Test so retry is one Enter.
- **Motion:** editor open = `gt-settings-editor-in` (opacity 0→1 + translateY −6→0, `--dur-short4` 200 ms, `--ease-standard-decelerate`) under `html.caps-motion`; close snaps (exit shorter than enter). Reduced-motion → opacity-only keyframes. Without `caps-motion` both snap.
- **Cleanup:** `wireLogSinkField` returns a disposer the screen's `destroy()` calls — drops the document capture-phase key trap and aborts any in-flight ping.
...
- **Platform notes:** switch knob travel is a `transform: translateX(28px)` with a `--dur-short2` / `--ease-standard` transition only under `html.caps-motion` (track colour always snaps; reduced-motion → no transition); no on-screen Back (remote-only); the destructive footer action has no confirm dialog but is guarded against Enter auto-repeat (parity with prior behaviour — revisit if accidental triggers occur).
- **Row states (hardened 2026-09-20):** rest · focused (light-fill inversion) · selected/on (switch `--on`, picker value) · selected+focused (switch stays `--accent` on the light fill) · **disabled** (`.gt-settings-item:disabled` → opacity 0.4; the D-pad skips it via `isNavFocusable`) · **in-flight** (`aria-busy="true"` → only the trailing hint dims; the row stays focusable so focus never jumps mid-request — used by "Send perf trace").
- **Async lists:** Plex Home profiles render a "Loading Plex Home profiles…" line, then the roster (0 → "Plex Home not available on this account."), or on failure a plain message + a focusable **Retry** action row; replies after leave or after a newer Retry are dropped (`alive` + request counter) and every re-render calls `invalidateFocusableCache()`. Watchlist rows: `refreshList` invalidates the focusable cache; rename re-focuses the new Rename button for the same list, delete lands on the Delete at the same position (else the last, else Create). Long / CJK names: profile rows wrap (`overflow-wrap`), watchlist names truncate with an ellipsis so the three buttons stay on the row; status messages wrap long URLs.
...
- **Hardened 2026-09-20 (focus + motion):** every rebuild (`renderTheme` / `renderContrast` / `renderTabs` / `rebuildPreview` / `renderEditor` / `renderEditorEmpty`) calls `invalidateFocusableCache()`. A pressed theme/contrast chip re-renders disabled, so focus lands on the nearest enabled neighbour (`focusChipNear`: next, else previous); a pressed tab re-focuses the same `[data-tab]`; a pressed role/tone control re-focuses its `[data-role]`/`[data-tone]` successor after the editor rebuild; Reset keeps focus on Reset. Enter auto-repeat is a no-op when the value is already active. **Theme/contrast switch transition:** the stage mock is rebuilt and fades in (`appearance-mock-in`, opacity 0→1, `--dur-short3`, `--ease-standard-decelerate`) under `html.caps-motion`; no colour or layout property animates; without the class it snaps. "Saved ✓" toggles `.appearance-save--show` (was mis-named `--visible` in CSS → never showed) with an opacity transition (`--dur-short3`, `--ease-standard`) under `caps-motion`. Mock titles/subtitles and list-item labels truncate on one line inside the stage (`min-width: 0` on the info columns); poster/avatar placeholders are plain fills, so missing art has no broken-image state. Dead CSS removed: `.appearance-controls`, `.appearance-control-*`, `.appearance-roles`; `.appearance-editor__section-label` renamed to the `.appearance-editor__heading` class the editor actually emits; `.appearance-segmented__hint` now styled.
...
- **Anatomy:**
  ...
  ```
- **Behaviour:** injected into `document.body` synchronously in `startApp()` before the first `navigate()` call. Dismissed (fade-out `--dur-medium2` 300ms opacity, accelerate, gated on `html.caps-motion`; DOM removal 350ms later) via `onFirstMount()` callback in `src/core/router.js` — fires once after the first screen factory returns (line after `entry.instance = routes[...](host, ...)` in `render()`). One-shot: `firstMountCallback` is nulled on first invocation.
...
- **Hardened 2026-09-20:** the `#detail-season-tabs` host composes `.row-scroll` (`position: relative; overflow-y: hidden`, rail padding overridden) so `focus.js` centres the focused tab when a show has more seasons than fit the width (30 seasons scroll; 1 season is a single pill). Labels come from module-level `seasonTabLabel`: positive index → "Season N", index 0 keeps its own title ("Specials"). **Season switch motion:** `loadShowEpisodes` keeps the outgoing cards in place at 50% (`.detail-episode-grid.is-loading`, `--dur-short3` / `--ease-standard-accelerate`) instead of flashing "Loading…", then `swapEpisodeGrid` plays `detail-episode-grid-enter` (opacity + `translateY(12px)` → 0, `--dur-short4` / `--ease-standard-decelerate`) on the new grid; `prefers-reduced-motion` swaps the keyframe for the opacity-only `gt-overlay-in`; without `html.caps-motion` the swap is a plain cut. A stale season response is dropped by `showEpisodesGen`.
...
- **Hardened 2026-09-20 (states, motion, concurrency):**
  - **Motion (all `html.caps-motion`-gated, transform/opacity only, snap without):** loading→content row reveal `.profile-picker-row--enter` = `gt-sheet-in` 200ms `--dur-short4` / `--ease-standard-decelerate` (JS adds the class on render; shared by the Jellyfin picker) · browsing→PIN panel `.profile-picker--pin-mode .profile-picker-pin` = `gt-sheet-in` 250ms `--dur-medium1` / `--ease-emphasized-decelerate`, exit instant · wrong-PIN `pin-shake` now gated and on tokens (`--dur-medium2` / `--ease-standard`; was ungated `0.35s ease`). `prefers-reduced-motion: reduce` → reveals become opacity-only (`screen-enter-fade`), shake off; the red `.pin-error` line stays as the feedback. Dropped the paint-property `transition: color` on `.profile-card-name` and the dead opacity transition on the row.
  - **`.profile-picker-pin[hidden] { display:none }`** — the panel's author `display:flex` had been beating the UA `[hidden]` rule, so an empty PIN panel reserved ~70px under the row in browsing mode.
  - **8+ profiles:** `.profile-picker-screen { overflow-y:auto }` — rows of 4 past the viewport scroll; `focus.js` `scrollNearestVertical` follows the focused card.
  - **Error state always has a target:** every load/bootstrap failure (launch path included, previously a bare red line with nothing focusable) renders `showLoadError` → `button#profile-retry` "Try again", focused; copy names the network/Plex recovery. `.status-msg.watch-status-error` now wins over the muted base colour (equal-specificity ordering bug; errors rendered grey on every picker/login screen).
  - **Concurrency:** load results are generation-guarded (`loadGen`) and the 20s load timer is cleared on destroy — a slow `fetchHomeUsers` from a screen the user left can no longer bootstrap + navigate Home; Back during "Verifying PIN" abandons that attempt (`switchGeneration += 1`).
  - **Content:** initials fallback is surrogate-pair safe (`initialsFromName`, shared with Jellyfin); empty title → "Profile".
...
- **Hardened 2026-09-20:** `choose()` runs once per screen life (`chosen` guard) — a double press on the remote can no longer run the auth reset + `navigate('pairing')` twice.
...
- **Hardened 2026-09-20:** label is a **2-line clamp** (`-webkit-line-clamp:2`, `overflow-wrap:break-word`, lh 1.3) instead of one-line ellipsis, so a Jellyfin URL or a long/CJK account name keeps its useful tail · 3+ rows: `.server-picker-screen { justify-content:flex-start; overflow-y:auto }` + `margin:auto` on first/last child (stays centred when short, scrolls when tall; `justify-content:center` alone clips the top on overflow) · 0 saved links → subtitle "No saved servers yet. Add one to get started." · `switchToLink` runs once (`switching` guard) · dropped the `color`/`border-color` transitions (paint props; ring + scale remain, transform-only).
...
- **Hardened 2026-09-20:** validation/network errors render **under the field** (see Login / Auth field → Error state) with the value preserved and focus moved to that field: empty address, unreachable/timeout/404/401/403/5xx (`describeJellyfinError(err, 'server'|'signin')`, exported), wrong password (401/403 → under Password). Server-side sign-in failures that aren't credentials stay on `#pw-status`. **Double-submit guards:** `connecting` / `signingIn` flags; the primary button reads "Connecting…" / "Signing in…" (`.is-busy`, `aria-busy`) while in flight and stays focusable (never `disabled`, so focus can't be dropped). **Stale probe guard:** `addressGen` bumps on every address edit; a probe of an older address that resolves late is ignored. Quick Connect: expired code (5 min) → back to the address step with "The Quick Connect code expired. Press Connect for a new one."; server with QC off → plain hint to use username & password. **Step change motion:** incoming `.login-step.is-active` = `gt-sheet-in` 200ms `--dur-short4` / `--ease-standard-decelerate` under `html.caps-motion`; outgoing is `display:none` (instant); reduced-motion → opacity-only.
...
- **Hardened 2026-09-20:** header spinner (same `createSpinner` as the Plex picker) + `profile-picker--loading` (row hidden) while users load / sign-in / bootstrap; row reveal shares `.profile-picker-row--enter`. No server configured → "Connect to a server" button (focused) instead of a dead end. Empty public list → renders the "Other user" tile *before* auto-opening the username modal (so the modal's return-focus is the tile, not `<body>`) and leaves the hint "No users are listed on this server (or it isn't reachable). Choose Other user to sign in by name." Error copy via `describeJellyfinError`; wrong password names the user and the retry. Initials via shared `initialsFromName`. Known gap: `fetchPublicUsers` swallows network errors (resolves `[]`), so "server unreachable" and "no public users" are indistinguishable here — the copy covers both.
