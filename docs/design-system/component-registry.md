...
- **Migrated:**
  ...
  - ✅ **player track-selector** + **media-info** → kit **side panel** (Direction=Right): surface-container panel on the right edge, scrim @60%, margin-based rhythm (Chrome53-safe). Player focus/key JS kept intact (well-tested `data-focus-zone` trap); only the presentation became the drawer. Width up-scaled via `--modal-sheet-max-w` for 10-ft; category chevrons retained (ratified).
    - **Height (ratified deviation from kit `h-full`):** the sheet is **content-sized between `--drawer-side-min-h` (320px) floor and the available safe height** (`max-height:100%` inside a `padding: --safe-y 0` container, vertically centred on the edge), NOT full-height — full-height left dead space below short lists. The **list (`.player-menu-list` / `.player-media-info-body`) is the flex scroll region** (`flex:1 1 auto; min-height:0; overflow-y:auto`, no fixed max-height); when items exceed the cap the list scrolls, header + footer stay pinned.
