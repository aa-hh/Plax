...
- **Keyboard/input contract (inline native inputs):** while a live `<input>`/`<textarea>` is the active field, the on-screen-keyboard editing keys act on the FIELD, not the app — **Left/Right move the cursor, Back/Backspace deletes a char**; Up/Down (and Back on an empty field) leave the field. Enforced centrally in `src/ui/focus.js` via a `window` capture-phase guard (`onEditableKeydown`) that claims these keys with `stopImmediatePropagation` before the geometric d-pad engine *and* the router's global Back handler (`src/core/router.js`) can hijack them; it tracks the active field so the contract survives the webOS keyboard pulling DOM focus to `<body>`. Regression history: the original per-screen handling was silently dropped when the d-pad engine was rewritten (`a72371d`, zone-graph → geometric); restored generically 2026-06-26 and verified on the B8 (cursor move + delete via both keyCode and `key`-string events; empty-field Back still navigates). The modal field (`openTextInputModal`) keeps its own equivalent scoped handler.
